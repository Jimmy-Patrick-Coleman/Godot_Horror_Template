...
@export var attack_range : float = 1.5 # attack range
@export var stop_range : float = 0.5 # range to stop before collision
...
func update (delta): # the equivalent of the physics function
	...
	if controller.player_distance < attack_range: # if the player is in the attack range end the game
